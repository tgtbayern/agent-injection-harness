...
const I18N = {
  zh: {
    ...
    mockBadge: "脚本客户端 · 不是模型输出",
    mockNote: "这一局由离线脚本客户端产生：约 200 行 if-else 加正则，没有任何模型参与。它的作用是把整条链路跑通（工具调用、防护拦截、重试、兜底），不是打得好。要看真实推理，配好网关后用 --model 跑。",
    ...
    hPhases: "本轮时序",
    hExposure: "暴露矩阵 · 每格一名玩家的一轮",
    fTable: "人数", fElapsed: "耗时",
    roundShort: (r) => `R${r}`,
    xState: { attacker: "投放者", hijacked: "选票被劫持", exposed: "读到载荷",
              blocked: "被防护拦截", benign: "读到良性说服", clear: "无事",
              dead: "已出局" },
    phaseNight: "夜", phaseSpeech: "昼 · 发言", phaseVote: "昼 · 投票",
    phaseCampaign: "昼 · 警长竞选", phaseLastWords: "遗言与枪响",
    sheriffIs: (p) => `警长：${p} 号`, sheriffNone: "警徽流局，无人当选",
    candidates: (l) => `上警：${l}`, stoodDown: "退选",
    lastWordsOf: (p) => `${p} 号 · 遗言`,
    shotLine: (a, b) => `${a} 号开枪带走 ${b} 号`,
    heldFire: (a) => `${a} 号没有开枪`,
    badgeLine: (a, b) => `${a} 号把警徽移交给 ${b} 号`,
    badgeTorn: (a) => `${a} 号撕毁了警徽`,
    badgeTag: "警徽",
    ...
    task: { speak: "发言回合", vote: "投票回合", campaign: "竞选回合",
            campaign_vote: "选警投票", last_words: "遗言回合",
            hunter_shoot: "猎人开枪", badge: "警徽移交" },
  ...
  en: {
    ...
    mockBadge: "scripted client · not model output",
    mockNote: "This game came from the offline scripted client: about 200 lines of if-else and regex, with no model involved. Its job is to exercise the pipeline -- tool calls, guard blocks, retries, fallbacks -- not to play well. For real reasoning, configure a gateway and run with --model.",
    ...
    hExposure: "exposure grid — one cell is one seat in one round",
    fTable: "seats", fElapsed: "elapsed",
    roundShort: (r) => `R${r}`,
    xState: { attacker: "planted a payload", hijacked: "ballot moved to the target",
              exposed: "read a payload", blocked: "a guard layer stopped it",
              benign: "read benign persuasion", clear: "nothing happened",
              dead: "out of the game" },
    phaseNight: "night", phaseSpeech: "day · speeches", phaseVote: "day · vote",
    phaseCampaign: "day · sheriff election", phaseLastWords: "last words & shots",
    sheriffIs: (p) => `sheriff: player ${p}`, sheriffNone: "tied; the badge went unclaimed",
    candidates: (l) => `standing: ${l}`, stoodDown: "stood down",
    lastWordsOf: (p) => `player ${p} · last words`,
    shotLine: (a, b) => `player ${a} shot player ${b}`,
    heldFire: (a) => `player ${a} held fire`,
    badgeLine: (a, b) => `player ${a} passed the badge to player ${b}`,
    badgeTorn: (a) => `player ${a} tore up the badge`,
    badgeTag: "badge",
    ...
    task: { speak: "speech turn", vote: "vote turn", campaign: "campaign turn",
            campaign_vote: "election ballot", last_words: "last words",
            hunter_shoot: "hunter's shot", badge: "badge succession" },
